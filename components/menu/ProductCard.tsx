...
export default function ProductCard({ product }: { product: Product }) {
  ...
  function handleAdd() {
    ...
    window.setTimeout(() => setShowFloat(false), 600)
  }
  ...
  return (
    /* Outer wrapper is overflow-visible so the float +1 can escape the card */
    <div className="relative">

      {/* Floating +1 micro-animation */}
      {showFloat && (
        <span
          key={floatKey}
          className="buns-float-up absolute top-10 right-5 z-20 text-base font-black text-buns-yellow select-none"
      ...
      <div
        className={[
          'relative bg-white border-2 rounded-2xl overflow-hidden flex flex-col',
          'transition-all duration-150 hover:-translate-y-1',
          popping ? 'buns-card-pop' : '',
          isFeatured
            ? 'border-buns-yellow shadow-[0_0_18px_rgba(255,212,0,0.22)] hover:shadow-[0_0_28px_rgba(255,212,0,0.42)]'
